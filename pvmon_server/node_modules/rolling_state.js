...
module.exports.state_change = function(ev,insert_event_final_callback) {
	...
	if(insert_event_final_callback) {
		...
		console.log('no callback for state change');
	}
...
module.exports.insert_event = function(ev,insert_event_final_callback) {
	...
	// pvmon var: exec
	// AND three extra params: (rs meaning rolling state)
	...
	// defaults
	Object.keys(rs_defaults).forEach(function(rs_param) {
		var rs_default_value = rs_defaults[rs_param];
		if(ev.hasOwnProperty(rs_param)) {
			if(typeof(ev[rs_param]) != 'number') {
				if(typeof(ev[rs_param]) != 'string' || ev[rs_param].match(/[^0-9]/)) {
					throw('param '+rs_param+' must be a number - current its a '+typeof(ev[rs_param]));
				}
			}
			if(ev[rs_param] != Math.round(ev[rs_param])) { throw('param '+rs_param+' must be an INTEGER number'); }
			if(ev[rs_param] < 1) { throw('param '+rs_param+' must be at least 1'); }
		} else {
			ev[rs_param] = rs_default_value;
		}
	});

	// now check stuff is OK
	if(ev.rs_length > 20) { throw('max value for rs_length is 20'); }
	if(ev.rs_max_crit > ev.rs_length) { throw('rs_max_crit cannot be over rs_length'); }
	if(ev.rs_max_warn > ev.rs_length) { throw('rs_max_warn cannot be over rs_length'); }
	if(ev.exec_interval > 86400) { throw('exec interval must be between 1sec and 1 day'); }

	if(ev.time - 3600 > (now() - (rs_timeframe)/1000)) { throw('event is over 1 hr in the future'); }
	if(ev.time + 86400 > (now() - (rs_timeframe)/1000)) { throw('event is over 24hr in the past'); }
	...
	if(old_state != new_state) {
		...
		module.exports.state_change(ev,insert_event_final_callback);
	} else {
